...
const errors = {
  ...
  loadDefault: function (service, cluster) {
    ...
    logger.log();
    logger.log('Load default errors @ ' + global.appName);
...
if (process.env.app_root) {
  ...
  let myErrors = process.env.my_errors_path || './app/errors.js';
  let errPath = path.resolve(process.env.app_root, myErrors);
  try {
    fs.accessSync(errPath, fs.F_OK);
    errors.load(require(errPath));
    logger.log();
    logger.log('Load my custom errors @ %s', global.appName);
  } catch (e) {
    logger.warn("File [%s] not exits. Skip errors loading.", errPath);
  }
