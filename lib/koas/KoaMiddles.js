...
class KoaMiddles {
  ...
  *errorsHandleBehind(next) {
    myTools.error("出错了，当前请求状态：" + this.status, this.url);
    switch (this.status) {
      case 400:
  ...
  *queryStringParser(next) {
  ...
  cloudeerProxy() {
    router.post('/cloudeer', function *() {
      ...
      this.body = yield Cloudeer.invokeCo(form.method, form.service, form.url, form.params);
    });
    if (process.env.debug) {
      router.get('/cloudeer/view', function *() {
        this.body = this.echo(Cloudeer.config);
      });
    }
    ...
    router.post('/cloudeer/get', function *() {
      ...
      //   }
      // }
      this.body = yield Cloudeer.invokeCo("GET", form.service, form.url, form.params);
    ...
    router.post('/cloudeer/post', function *() {
      ...
      this.body = yield Cloudeer.invokeCo("POST", form.service, form.url, form.params);
    });
    router.get('/inner/cloudeer/mnt-server-info', function *() {
      this.echo({
        cpus    : os.cpus(),
        uptime  : os.uptime(),
    ...
    router.get('/inner/cloudeer/mnt-process-info', function *() {
      this.echo({
  ...
  *authenticate(next) {
    ...
    if (Cloudeer.authUris.indexOf(authCode) < 0) {
      ...
      if (authCode.startsWith('/inner')) {
        ...
        var rights = yield Cloudeer.invokeCo("GET", "cloudarling", "/open/account/rights",
          {
