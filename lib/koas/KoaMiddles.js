...
const Cloudeer = require('../rpc/Cloudeer');
...
class KoaMiddles {
  constructor() {
    myTools.consoleLog('Koa middlewares inited.');
  }
   ...
   * @returns {*}
   */
  autoRouters(watchDir, app) {
    myTools.consoleLog("===========================");
    watchDir = watchDir || ['/api/open', '/api/admin', '/api/inner'];
    myTools.consoleLog('Auto router is ready. Current route folder are ' + watchDir);
    const autoRouter = new AutoRouter({ watchDir, app });
    autoRouter.bindRouter();
