...
class KoaMiddles {
  ...
  autoRouters(watchDir) {
    ...
    watchDir.forEach(function (e) {
      ...
      try {
        ...
        logger.warn("目录 %s 不可访问。", routers_dir);
      }
      if (rightAccess) {
  ...
  *authenticate(next) {
    ...
    if (Cloudeer.authUris.indexOf(authCode) < 0) {
      ...
      if (!jRights.hasOwnProperty("errno")){
        throw errors.CUSTOM("错误的输出，cloudarling 微服务的权限（/open/account/rights）数据格式应该包含 errno。");
      }
      if (jRights.errno != 0) {
