...
function KoaApplication(isDebug) {
  ...
  app.use(koaMiddle.jsonValidator(config.schemaPath, config.controllerDirs).schemaValidator);
  ...
  var router = koaMiddle.autoRouters(config.controllerDirs);
  ...
  var cloudeer = new Cloudeer({
    cloudeerUri: config.cloudeer.serviceHost || config.cloudeerUrl,
    myHost     : config.cloudeer.myHost || config.myIp,
