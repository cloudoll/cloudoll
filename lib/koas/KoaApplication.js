...
function KoaApplication(options) {
  ...
  if (config.cloudeer_tcp) {
    var CloudeerClient = require('../rpc/CloudeerClient');
    let client         = new CloudeerClient({
      cloudeer_server: config.cloudeer_tcp.server,
      app_name       : process.env.app_name,
      app_port       : xport,
      app_host       : config.my_host || config.my_ip || process.env.my_host || tools.getLocalIp(),
      username       : config.cloudeer_tcp.username,
      password       : config.cloudeer_tcp.password,
      not_a_consumer : config.cloudeer_tcp.not_a_consumer || false
    });
    client.startService();
  }
