...
function KoaApplication(isDebug) {
  ...
  process.env.debug = (isDebug || config.debug) ? "true" : "";
  var koa           = require('koa');
  ...
  if (!config.koa_middles_forbidden.authenticate) {
    console.log("===========================");
  ...
  var cloudeer = new Cloudeer({
    cloudeerUri: config.cloudeer.server || config.cloudeer_url || tools.getLocalIp(),
    myHost     : config.my_host || config.my_ip,
