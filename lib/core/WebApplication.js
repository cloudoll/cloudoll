...
`);
    ...
    startService() {
        if (global.debug) {
            this.router = new require('koa-router')(); //this is stupid, to fix app.router undefined.
            return this.startDev();
        } else {
            this.init();
            this.printAppInfo();
    ...
    startDev() {
        if (cluster.isMaster) {
            ...
            const workerNow = cluster.fork();
            console.log(`Main worker ${workerNow.process.pid} starting...`);

            cluster.on('exit', (worker, code, signal) => {
                // console.log(code, signal);
                code === 14 && cluster.fork();
            });
            this.printAppInfo();
        } else {
            this.init();
        ...
        this.watchWhenDev(path.resolve("./app/"));
        this.watchWhenDev(path.resolve("./config/"));
    }

    watchWhenDev(dir) {
        if (!global.debug || cluster.isMaster) {
            return;
        }

        fs.watch(dir, { recursive: true }, (eType, file) => {
            if (eType === "change" && (file.endsWith('.js') || file.endsWith('.json'))) {
                // active = true;
                logger.warn(`The file [${file}] has been changed, reloading...`);
                process.exit(14);
