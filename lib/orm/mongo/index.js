...
var ezmongo = module.exports = {
  ...
  findOneAndInc    : function (collectionName, doc, query, options) {
    return function (callback) {
      ...
      var collection = ezmongo.db.collection(collectionName);
      collection.updateOne(query, {$inc: doc}, options, function (err, r) {
        if (r.result.ok == 1) {
          callback(err, doc);
        } else {
          callback(errors.CUSTOM("自增失败."));
        }
      });
    }
  },
  ...
  findOneAndIncWithLock    : function (collectionName, doc, query, options) {
    return function (callback) {
      ...
      var collection = ezmongo.db.collection(collectionName);
      collection.findOneAndUpdate(query, {$inc: doc}, options, function (err, r) {
        if (r.result.ok == 1) {
          callback(err, doc);
        } else {
          callback(errors.CUSTOM("自增失败."));
