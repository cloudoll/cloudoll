...
const ezmysql = module.exports = {
  ...
  list: async (table, conditions) => {
    ...
    return ezmysql.query(sql, conditions.params);
  ...
  save: async (table, model) => {
    if (model.hasOwnProperty('id') && ~~model.id > 0) {
      return ezmysql.update(table, model);
    } else {
      return ezmysql.insert(table, model);
    }
  },
  ...
  loadByKV: async (table, key, value) => {
    return ezmysql.load(table, {
      where: key + " = ?",
      params: [value]
    });
  },
  loadById: async (table, id) => {
    return ezmysql.loadByKV(table, "id", id);
  },
