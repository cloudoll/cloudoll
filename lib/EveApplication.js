...
class EveApplication extends egg.Application {
  constructor(options) {
    ...
    this.config.eve = this.config.eve || {};
  ...
  bindRpc() {
    ...
    if (this.config.eve.selfProxy || this.config.eve.providers || this.config.eve.regServer) {
      ...
      this.rpcEve = function*(verb, service, url, params) {
        return yield this.eveRpc.request(verb, service, url, params);
      };
      this.getEve = function*(service, url, params) {
        return yield this.eveRpc.request("GET", service, url, params);
      };
      this.postEve = function*(service, url, params) {
        return yield this.eveRpc.request("POST", service, url, params);
      };
