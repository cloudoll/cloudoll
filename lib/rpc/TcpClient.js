...
function TcpClient(options) {
  ...
  this.heartBeatInterval  = 5000; //超时时间
        ...
        if (jsonInfo.errno != 0) {
          ...
          switch (jsonInfo.cmd) {
            case 'login':
              ...
              // }
              heartBeat(this.socket, this.heartBeatInterval); //开始发送心跳
              break;
            case 'reg-service':
...
const onGetServices = function (services) {

};

const heartBeat = function (socket, interval) {
  tools.info("我的心开始 砰砰 跳...");
  socket.heartBeat = setInterval(function () {
    sendJson(socket, {cmd: 'ping'});
  }, interval || 5000);
};
